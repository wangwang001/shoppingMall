...
;(function ($, window, document, undefined) {
  ...
  Faqrobot.prototype = {
                ...
                if(phoneWidth == 375){
                    if(phoneHeight > 635){
                      if(window.location.host=='jdyw.faqrobot.cn'){//判断是不是ppmoney
                        ...
                        var chatStyle = '.front .chatHeight{height:'+parseInt($(document).height()-485)+'px !important}';
                      }
                    }else{
      ...
      return This.request({
        ...
        callback: function (data) {
          ...
          //This.sayHello(data)//欢迎语
           /**add by zhaoyuxing app闪退 时不显示欢迎语和问题引导；
            * 注意：开启闪退功能时，需要注释sayHello() welcomeguideQue()方法；关闭时取消注释
           * */
          ...
          //不闪退中已经加入了提示欢迎语功能，所以这边注释掉，提交人：顾荣   2018/1/12
          //This.welcomeguideQue(data)
    ...
    robotName:'',//当闪退时，获取机器人名称；闪退时不调用sayHello方法，无法获得机器人名称
    //taskId=406 app闪退处理
    robotName:'',//用于存储机器人名称
    flashOutDeal:function(data){
      ...
      for (var i = 0; i<data.talkMessageList.length; i++) {
          ...
          // recordData += (data.talkMessageList[i].askMsg ? This.customHtml(This.replaceFace(data.talkMessageList[i].askMsg), data.talkMessageList[i].askTime) : '') + ((data.talkMessageList[i].ansMsg || '') ? This.robotHtml(_data) : '')
          var obj={
    ...
    */
    ...
    robotHtml: function (data, index, hotQue) {//hotQue代表欢迎语下面的热点问题
      ...
      if (data.webConfig) {//欢迎语结构
        ...
        if (data.robotAnswer[index]) {
           ...
           */
          if (data.robotAnswer[index].aId > 1000&&data.robotAnswer[index].serviceType!="seat") {//aId!=0->需要评价
            //%%
      ...
      if (question) {//问题不为空
        ...
        if (question.indexOf('%我要发文件%') + 1) {//发文件
          ...
          if (This.robot.html || queParam) {// 当 This.robot.html 不为空时，走模拟答案
            ...
            */
            /**
             * 下先后在发送问题，先调取s=p初始化，再调用s=kl和s=aq恢复问答
             */
    ...
    initComment: function () {
      ...
      $commentSendBtnId.on('click.FA', function () {
        /*
         * taskId = 501 意见反馈选择不满意，填写内容回车后，无法提交
         * 解决：点击提交时把内容去除空格回车
         * */
