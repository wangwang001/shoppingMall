    ...
    ;(function($, window, document, undefined) {
        ...
        var plugName = "faqrobot",
            defaults = {
				...
				relateQueId: 'relateQueId',//相关问题Id
        ...
        Faqrobot.prototype = {
            ...
            sayHello: function(data) {
            	console.log(this.robotHtml(data))
            ...
            robotHtml: function(data, index) {
                ...
                if(data.webConfig) {//欢迎语结构
                	...
                	if(data.robotAnswer[index]) {
	                    ...
	                    this.relateQue(data.robotAnswer[index]);//智能搜索
            ...
            askQue: function() {
			 ...
			 //相关问题
            relateQue: function(data) {
				this.options.relateQueCallback(data);
                if(!this._options.relateQueId) {//不配置直接返回
                    return;
                }

                var $relateQueId = this.$obj.$relateQueId = $('#'+ this.options.relateQueId),//相关问题Id
                    relateQue = data.relateLessList;
				if(relateQue && relateQue.length>0){
					$relateQueId.empty().append(this.queHtml(relateQue));
				}
            },
            // 智能搜索
