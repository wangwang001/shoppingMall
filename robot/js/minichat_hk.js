//;(function() {//匿名函数，防止污染
    ...
    **/
    ...
    ;(function($, window, document, undefined) {
        ...
        Faqrobot.prototype = {
            init: function() {
				//设置变量setInputTop来判断定制页面是否需要此方法，sourceId判断渠道。默认开启此方法，如果不需要在app.js中设置变量setInputTop为false
				if((!this.options.setInputTop)&&(this.options.sourceId == 3)){
					
				}else{
					this.setInputTop()//键盘遮挡问题
				}
        ...
        if(this.isiOS){
            if(version1 == '11'&&version2 > 0&&version2 < 3){
                ...
                if(phoneWidth == 375){
                    ...
                    var chatStyle = '.front .chatHeight{height:'+parseInt($(document).height()-325)+'px !important}';
                    }
                }
                ...
                $('#'+this.options.inputCtnId).on('focus', function () {
                  $('.chatScroll').addClass('chatHeight');
					  ...
					  $('.front').height(($('.chatScroll').height()+52));
            ...
            robotHtml: function(data, index, hotQue) {//hotQue代表欢迎语下面的热点问题
				...
				if(hotQue){
					var topAskHtml='';
					if(data.guideQuestions && data.guideQuestions.length>1){
						topAskHtml+='<div class="guessYOu">'
						for(var i=0; i< data.guideQuestions.length; i++) {
							if(i==0){
								topAskHtml+='<div>我猜你想问</div>';
							}
			...
			recommendUrl:function(data,index){
				 ...
				 if(data.robotAnswer[index].thirdUrl && data.robotAnswer[index].thirdUrl.url){
					...
					$('#'+ this.options.thirdUrlId+' iframe').attr('src',data.robotAnswer[index].thirdUrl.url);
				 }else{
