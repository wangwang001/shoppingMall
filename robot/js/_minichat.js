    ...
    ;(function($, window, document, undefined) {
        ...
        Autocomplete.prototype = {
            ...
            event: function() {
            	...
            	//取消
            	$(document).on(This.eventType +'.AU', function(e) {
    ...
    ;(function($, window, document, undefined) {
        ...
        Face.prototype = {
            	...
            	for(var key in this.obj.face) {
            		...
            		if(key == this.obj.faceType[0]) {//选择表情
            			...
            			for(var i=0; i<this.options.rowNum*this.obj.maxNum_y; i++) {
            				...
            				if(i < mood.length) {
            					srcHtml = '<img src="'+ this.options.src + i +'.'+ this.obj.faceType[1] +'">';
            					title = mood[i][0];
            					mark = mood[i][1];
            				}
            ...
            event: function() {
            	...
            	//选择表情
            	This.options.targetEl.on(eventType +'.FA', '.FA_moodCtn', function() {
            		var val = This.$el.val(),
        		...
        		//显隐
        		$(document).on(eventType +'.FA', function(e) {
    ...
    **/
    ...
    ;(function($, window, document, undefined) {
        ...
        Faqrobot.prototype = {
            ...
            //客户结构
            customHtml: function(word) {
                var html = '';

                html = this.options.khHtml.replace(/%khPic%/g, this.robot.khPic).replace(/%askWord%/g, word).replace(/%formatDate%/g, this.getFormatDate());
            ...
            askQue: function() {
                ...
                if(question) {//问题不为空
                    ...
                    if(question.indexOf('%我要发文件%')+1) {//发文件
                    	...
                    	if(This.robot.html) {
                    		data = JSON.parse('{"robotAnswer":[{"ansCon":"'+ (This.robot.html).replace(/"/g, '\'') +'"}]}');
            ...
            upFile: function() {
            	var This = this;
            	var $file = $('<input type="file" class="FA_file" multiple="multiple">').hide().appendTo('body');

            	$('#'+ this.options.upFileModule.triggerId).on(this.eventType, function() {
            		$file.trigger('click');
            	});
            	H5_upload('../../servlet/AQ?s=uf', this.options.upFileModule.maxNum, $file, this.$obj.$chatCtnId, function(ran) {
        			...
        			$('#'+ This.options.sendBtnId).trigger(This.eventType);
            ...
            initInput: function() {
            	...
            	if(MN_Base.isPC()) {
            		...
            		$sendBtnId.on('touchend.FA', function() {
            		    This.askQue();
            		});
            	}
            },
            ...
            preview: function() {
            	...
            	$('.FA_previewImgCtn, .FA_previewClose').on(This.eventType +'.FA', function(e) {
	            		if(e.target.className != 'FA_previewImg') {
	            			$('.FA_previewCtn').hide().find('img').remove();
	            		}
            		});
            	if(This.options.noView != 'all') {
            		This.$obj.$chatCtnId.on(This.eventType +'.FA', 'img', function(e) {
            			var noViewArr = [];
    ...
    ;MN(function() {//使能获取到加载完毕的DOM
        ...
        var $minichatCss = MN('<link rel="stylesheet" href="'+ prefix[0] +'/'+ prefix[1] +'/css/_minichat.css"/>').appendTo('head');
    	...
    	function minichatJs() {
	    	...
	    	if(skinNum) {
	    		MN('<link rel="stylesheet" href="'+ prefix[0] +'/'+ prefix[1] +'/css/qqskin'+ skinNum +'.css"/>').appendTo('head');//css路径 待完善
	    	}

	    	//侧边栏
	    	var $MN_sideBar = MN('<div class="MN_sideBar"><i class="MN_sideIcon"></i><div class="MN_sideWord">在线技术咨询</div></div>').appendTo('body');
	    	...
	    	//头部左侧
	    	var $MN_headLeftCtn = MN('<div class="MN_headLeftCtn"><img class="MN_logoSrc" id="MN_logoSrc"><div class="MN_logoWord" id="MN_logoWord"></div></div>').appendTo($MN_headCtn);
	    	...
	    	//底部底部
	    	var $MN_footFootCtn = MN('<div class="MN_footFootCtn"></div').appendTo($MN_footCtn);
	    	//底部身体
	    	...
	    	//表情按钮
	    	var $MN_faceBtn = MN('<span class="MN_faceBtn" id="MN_faceBtn"></span>').css({
	    	}).appendTo($MN_footTopCtn);
    	    ...
    	    var Face = $MN_inputCtn.face({
    	        src: prefix[0] +'/'+ prefix[1] +'/src/yun/',//表情路径
    	        rowNum: 7,//每行最多显示数量，此属性不适用于常用语
    	        ctnAttr: [0, 30, 40, 40],//[left bottom width height] 表情框相对targetEl位置和里面的表情格子宽高  要写单位
    	        triggerEl: $MN_faceBtn,//触发按钮(不存在则自己生成，不要由a包裹)
    	        targetEl: $MN_footTopCtn,//父级参照物(用于appendTo和定位)
    	        hideAdv: true,//是否隐藏广告
    	        callback: function() {
    	        },
    	    });
	    	...
	    	var FAQ = new Faqrobot({
	            ...
	            //jid: 0,//自定义客服客户图标
				//robotName: 'FaqRobot',//机器人名称
				logoUrl: prefix[1] +'/'+'images/logo.png',//logo地址
				logoId: 'MN_logoSrc',//logo地址
    			webNameId: 'MN_logoWord',//公司名称Id
	            //userInfoId: 'userInfoId',//用户信息Id
	            kfPic: prefix[1] +'/'+'images/robot.png',  //客服图标
	            khPic: prefix[1] +'/'+'images/user.png', //客户图标
                formatDate: '%hour%:%minute%:%second% %year%-%month%-%date%',//配置时间格式(默认10:42:52 2016-06-24)
